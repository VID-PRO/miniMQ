...
public:
    ...
    // Register the NCM interface/endpoints/strings with the shared USB object.
    // Must be called AFTER the USBClass USB global has been constructed but
    // BEFORE USB.begin() builds the descriptor, i.e. from initVariant().
    // Doing this in the constructor is unsafe: the eth global may be
    // constructed before the USB global, whose constructor then wipes the list.
    void usbRegisterInterfaces();
