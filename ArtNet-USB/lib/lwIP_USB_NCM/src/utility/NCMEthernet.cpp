...
// Default node MAC for the NCM descriptor. The host's copy of the interface MAC
// is this with the last byte inverted (see below), so the host MAC string is
// 02:00:00:00:00:FE. Keep this in sync with main.cpp's NCM_MAC so the lwIP
// netif MAC, the USB descriptor and ArtPollReply all agree.
static const uint8_t ncm_default_mac[6] = {0x02, 0x00, 0x00, 0x00, 0x00, 0x01};
...
void NCMEthernet::usbRegisterInterfaces() {
    // Register the NCM USB interface from initVariant(), which the framework
    // calls between C++ global construction and USB.begin(). This is the one
    // place where the USBClass USB global is guaranteed to be fully
    // constructed AND the descriptor has not been built yet, so the very first
    // enumeration the host sees already contains CDC + NCM together.
    //
    // NOTE: do NOT move this into the constructor. The "NCMEthernetlwIP eth"
    // global in main.cpp may be constructed BEFORE the "USBClass USB" global
    // (link order), and USB's constructor zeroes _interfaces/_endpointIn/_endpointOut
    // afterwards, silently dropping everything registered here.
    //
    // The upstream library registers in begin(), which forces a
    // USB.disconnect()/connect() re-enumeration shortly after boot. Hosts
    // (notably macOS) frequently keep the first, CDC-only descriptor they saw
    // and never create the NCM Ethernet interface. Registering up front
    // eliminates that dance entirely.
    memcpy(tud_network_mac_address, ncm_default_mac, 6);
    ...
    // mac address we give to the PC; must be different than our own
    uint8_t len = 0;
...
void NCMEthernet::usbInterfaceCB(int itf, uint8_t *dst, int len) {
    ...
    // macOS NCM driver workaround (Apple Developer Forums 824727, Apple bug
    // r.170072016): TinyUSB emits bmNetworkCapabilities = 0x00 in the NCM
    // functional descriptor, a value macOS's com.apple.driver.usb.cdc.ncm
    // refuses to activate. Advertising SetEthernetPacketFilter (bit0 = 0x01)
    // makes macOS probe the filter, and when the device stalls the request
    // macOS proceeds and hands out a working interface. Layout of the 6-byte
    // NCM functional descriptor (dcbNCM starts at offset 40):
    //   40: bLength=0x06  41: bDescriptorType=0x24  42: subtype=0x1A
    //   43..44: bcdNCM(0x0100)                       45: bmNetworkCapabilities
    static_assert(TUD_CDC_NCM_DESC_LEN >= 46, "NCM desc too short");
    desc[45] = 0x01;
